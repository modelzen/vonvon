...
class SessionDB:
    ...
    def replace_messages(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        """Replace all messages in a session with a new set (e.g. after compression).

        Performs an atomic DELETE + INSERT so the session history reflects the
        compressed conversation. Used by vonvon backend's context compression flow.
        """
        def _do(conn):
            ...
            for msg in messages:
                ...
                conn.execute(
                    """INSERT INTO messages (session_id, role, content, tool_call_id,
                       tool_calls, tool_name, timestamp, reasoning, reasoning_details,
